...
export class ConfluenceService {
  ...
  getAllConfluences() {
    return this.httpClient.get<ConfluenceDto[]>(`${environment.CONFLUENCE_API_ENDPOINT}/confluence`, {
      responseType: 'json',
    });
  }
  ...
  getConfluenceById(id: number) {
    return this.httpClient.get<ConfluenceDto>(`${environment.CONFLUENCE_API_ENDPOINT}/confluence/${id}`, {
      responseType: 'json',
    });
  }

  updateConfluence(id: number, form: ConfluenceUpdateDto) {
    return this.httpClient.put<ConfluenceDto>(`${environment.CONFLUENCE_API_ENDPOINT}/confluence/${id}`, form, {
      responseType: 'json',
    });
  }

  addSubscribeSource(form: SubscribeSourceCreationDto) {
    return this.httpClient.post<SubscribeSourceDto>(`${environment.CONFLUENCE_API_ENDPOINT}/subscribe_source`, form, {
      responseType: 'json',
    });
  }
