    ...
    <p-dataView #dv [value]="confluences">
      <ng-template pTemplate="header">
        <div class="flex justify-content-between align-items-center">
          ...
          <p-button label="Add" (click)="addConfluence()" icon="pi pi-plus" [outlined]="true" />
        </div>
      </ng-template>
      <ng-template let-confluence pTemplate="list">
        <div class="grid grid-nogutter">
          <div class="col-12" *ngFor="let item of confluences; let first = first">
            <div
              class="flex flex-row align-items-start p-4 gap-4"
              [ngClass]="{ 'border-top-1 surface-border': !first }"
            >
              <p-avatar [label]="item.name.slice(0, 1).toUpperCase()" styleClass="mr-2" size="xlarge"></p-avatar>
                ...
                <div class="flex flex-column align-items-center sm:align-items-start gap-2">
                  ...
                  <div class="flex align-items-center gap-3">
                    ...
                    <p-tag
                      ...
                      [style]="{
                        background: getSeverityText(item) === 'active' ? 'var(--blue-500)' : 'var(--gray-500)'
                      }"
                    ></p-tag>
                  </div>
                </div>
...
export class DashboardComponent implements OnInit {
  ...
  getSeverity(item: ConfluenceDto) {
    if (item.mux_content) {
      return 'success';
    }
    return 'info';
