...
pub fn parse_server_tld<'a, 'b>(config_name: &'a str, name: &'b str) -> Result<ServerTld<'b>, ConfigError> {
    let proxy_server =
        parse_domain_name(&name);

    if let Err(err) = &proxy_server {
        if err.kind() == addr::error::Kind::NumericTld {
            let addr = IpAddr::parse_ascii(name.as_bytes()).map_err(|e| ConfigError::ProxyServerIpInvalid {
